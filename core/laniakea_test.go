...
func getPluginDir(t *testing.T) string {
	...
	match, err := regexp.MatchString(coreDirRegexp, dir)
	if err != nil {
...
func initLani(t *testing.T, shutdownInterceptor *intercept.Interceptor, readySigChan chan struct{}, wg *sync.WaitGroup, tempDir string) {
	defer wg.Done()
	...
	t.Log("Waiting for password. Use `lanicli setpassword` to set a password for the first time, " +
...
// unlockLani is a helper function to unlock the Laniakea Daemon
func unlockLani(ctx context.Context, unlockerClient lanirpc.UnlockerClient) error {
	_, err := unlockerClient.SetPassword(ctx, &lanirpc.SetPwdRequest{
...
func TestUnlockerGrpcApi(t *testing.T) {
	...
	wg.Add(1)
	go initLani(t, shutdownInterceptor, readySignal, &wg, tempDir)
	<-readySignal
	ctx := context.Background()
	...
	for _, c := range unlockerTestCasesOne {
		t.Run(c.caseName, func(t *testing.T) {
			switch c.command {
			case chngePwdCmd:
				_, err := client.ChangePassword(ctx, &lanirpc.ChangePwdRequest{
			...
			case loginCmd:
				_, err := client.Login(ctx, &lanirpc.LoginRequest{
					Password: c.oldPwd,
				})
				if c.expectedErr == nil {
					if err != c.expectedErr {
			...
			case setPwdCmd:
				_, err := client.SetPassword(ctx, &lanirpc.SetPwdRequest{
					Password: c.oldPwd,
				})
				if c.expectedErr == nil {
					if err != c.expectedErr {
...
type bakeMacCases struct {
	caseName    string
	bakeMacReq  *lanirpc.BakeMacaroonRequest
	expectedErr error
...
var (
	bakeMacTestCases = []bakeMacCases{
		{"bake-mac-empty-permissions", &lanirpc.BakeMacaroonRequest{}, ErrEmptyPermissionsList, func(macHex string, tlsDialOpt grpc.DialOption) (*grpc.ClientConn, error) {
			return nil, nil
		}},
		{"bake-mac-invalid-permission-entity", &lanirpc.BakeMacaroonRequest{
			Timeout:     int64(-10),
			TimeoutType: lanirpc.TimeoutType_DAY,
		...
		{"bake-mac-invalid-permission-action", &lanirpc.BakeMacaroonRequest{
			Timeout:     int64(-10),
			TimeoutType: lanirpc.TimeoutType_DAY,
			Permissions: []*lanirpc.MacaroonPermission{
				&lanirpc.MacaroonPermission{
		...
		{"bake-mac-10-sec-timeout", &lanirpc.BakeMacaroonRequest{
			Timeout:     int64(10),
			...
			_, err = client.AdminTest(ctx, &lanirpc.AdminTestRequest{})
			if err != nil {
				return nil, e.Wrap(err, "unable to invoke admin test command")
			}
			_, err = client.BakeMacaroon(ctx, &lanirpc.BakeMacaroonRequest{})
			if err == nil {
...
// TestLaniGrpcApi tests the Laniakea API service
func TestLaniGrpcApi(t *testing.T) {
	// temp dir
	tempDir, err := ioutil.TempDir("", "integration-testing-")
	if err != nil {
		t.Fatalf("Error creating temporary directory: %v", err)
	...
	defer conn.Close()
	unlockerClient := lanirpc.NewUnlockerClient(conn)
	unAuthedLaniClient := lanirpc.NewLaniClient(conn)
	err = unlockLani(ctx, unlockerClient)
	if err != nil {
		t.Fatalf("Could not set Laniakea password: %v", err)
	}
	time.Sleep(1 * time.Second)
	// now we get LaniClient
	...
	laniClient := lanirpc.NewLaniClient(authConn)
	// test command
	t.Run("test-command", func(t *testing.T) {
		resp, err := laniClient.TestCommand(ctx, &lanirpc.TestRequest{})
		if err != nil {
	...
	t.Run("admin-test", func(t *testing.T) {
		resp, err := laniClient.AdminTest(ctx, &lanirpc.AdminTestRequest{})
		if err != nil {
	...
	for _, c := range bakeMacTestCases {
		t.Run(c.caseName, func(t *testing.T) {
			_, err := laniClient.BakeMacaroon(ctx, c.bakeMacReq)
			if c.expectedErr == nil {
				if err != c.expectedErr {
	...
	t.Run("stop-daemon", func(t *testing.T) {
		_, err := laniClient.StopDaemon(ctx, &lanirpc.StopRequest{})
		if err != nil {
			st, ok := status.FromError(err)
			if !ok {
	...
	t.Run("set-temperature", func(t *testing.T) {
		_, err := unAuthedLaniClient.SetTemperature(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("set-pressure", func(t *testing.T) {
		_, err := unAuthedLaniClient.SetPressure(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("start-recording", func(t *testing.T) {
		_, err := unAuthedLaniClient.StartRecording(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("stop-recording", func(t *testing.T) {
		_, err := unAuthedLaniClient.StopRecording(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("subscribe-datastream", func(t *testing.T) {
		_, err := unAuthedLaniClient.SubscribeDataStream(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("load-testplan", func(t *testing.T) {
		_, err := unAuthedLaniClient.LoadTestPlan(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("start-testplan", func(t *testing.T) {
		_, err := unAuthedLaniClient.StartTestPlan(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("stop-testplan", func(t *testing.T) {
		_, err := unAuthedLaniClient.StopTestPlan(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("insert-roi", func(t *testing.T) {
		_, err := unAuthedLaniClient.InsertROIMarker(ctx, &proto.Empty{})
		st, ok := status.FromError(err)
		if !ok {
...
func TestPluginAPI(t *testing.T) {
	...
	defer authConn.Close()
	plugClient := lanirpc.NewPluginAPIClient(authConn)
	t.Run("start record-plugin not registered", func(t *testing.T) {
		_, err := plugClient.StartRecord(ctx, &lanirpc.PluginRequest{Name: "invalid-plugin-name"})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("start record-plugin invalid state", func(t *testing.T) {
		_, err := plugClient.StartRecord(ctx, &lanirpc.PluginRequest{Name: "test-timeout-plugin"})
		if err != nil {
			t.Errorf("Unexpected error when calling StartRecord: %v", err)
		}
	...
	t.Run("start record-already recording", func(t *testing.T) {
		_, err := plugClient.StartRecord(ctx, &lanirpc.PluginRequest{Name: "test-rng-plugin"})
		if err != nil {
			t.Errorf("Unexpected error when calling StartRecord: %v", err)
		}
		_, err = plugClient.StartRecord(ctx, &lanirpc.PluginRequest{Name: "test-rng-plugin"})
	...
	t.Run("start record-plugin error", func(t *testing.T) {
		_, err := plugClient.StartRecord(ctx, &lanirpc.PluginRequest{Name: "test-error-plugin"})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("start record-plugin timeout", func(t *testing.T) {
		pluginState, err := plugClient.GetPlugin(ctx, &lanirpc.PluginRequest{Name: "test-timeout-plugin"})
		if err != nil {
			t.Errorf("Unexpected error when calling GetPlugin: %v", err)
		}
		if pluginState.State != lanirpc.PluginState_UNRESPONSIVE {
			t.Errorf("Plugin in unexpected state: %v", pluginState.State)
		}
	})
	t.Run("start plugin-unregistered plugin", func(t *testing.T) {
		_, err := plugClient.StartPlugin(ctx, &lanirpc.PluginRequest{Name: "invalid-plugin"})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("start plugin-already started", func(t *testing.T) {
		_, err := plugClient.StartPlugin(ctx, &lanirpc.PluginRequest{Name: "test-error-plugin"})
		st, ok := status.FromError(err)
		if !ok {
			t.Errorf("Unexpected error type coming from StartPlugin gRPC method: %v", err)
		}
	...
	t.Run("start plugin-valid", func(t *testing.T) {
		_, err := plugClient.StopPlugin(ctx, &lanirpc.PluginRequest{Name: "test-rng-plugin"})
		if err != nil {
			t.Errorf("Unexpected error when calling StopPlugin: %v", err)
		}
		_, err = plugClient.StartPlugin(ctx, &lanirpc.PluginRequest{Name: "test-rng-plugin"})
		if err != nil {
			t.Errorf("Unexpected error when calling StartPlugin: %v", err)
		}
	})
	t.Run("stop plugin-unregistered plugin", func(t *testing.T) {
		_, err := plugClient.StopPlugin(ctx, &lanirpc.PluginRequest{Name: "invalid-plugin"})
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("command-unregistered plugin", func(t *testing.T) {
		stream, _ := plugClient.Command(ctx, &lanirpc.ControllerPluginRequest{Name: "unregistered-plugin"})
		_, err := stream.Recv()
		st, ok := status.FromError(err)
		if !ok {
	...
	t.Run("Command-empty frame", func(t *testing.T) {
		stream, _ := plugClient.Command(ctx, &lanirpc.ControllerPluginRequest{Name: "test-ctrl-plugin", Frame: &proto.Frame{}})
		_, err := stream.Recv()
		st, ok := status.FromError(err)
		if !ok {
			t.Errorf("Unexpected error type coming from Command gRPC method: %v", err)
	...
	t.Run("command-single return frame", func(t *testing.T) {
		...
		stream, err := plugClient.Command(ctx, &lanirpc.ControllerPluginRequest{Name: "test-ctrl-plugin", Frame: &proto.Frame{
			Source:    "client",
