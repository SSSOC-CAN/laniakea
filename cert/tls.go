...
func TLSConfFromCert(certDat tls.Certificate) *tls.Config {
	return &tls.Config{
...
func GetTLSConfig(certPath, keyPath string) ([]grpc.ServerOption, []grpc.DialOption,
	...
	certData, parsedCert, err := cert.LoadCertificate(certPath, keyPath)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	...
	if time.Now().After(parsedCert.NotAfter) {
		...
		certData, _, err := cert.LoadCertificate(certPath, keyPath)
		if err != nil {
			return nil, nil, nil, nil, err
		}
	}
	tlsCfg := TLSConfFromCert(certData)
	restCreds, err := grpc.credentials.NewClientTLSFromFile(certPath, "")
	if err != nil {
		return nil, nil, nil, nil, err
	}
	cleanUp := func() {}
	serverCreds := grpc.credentials.NewTLS(tlsCfg)
