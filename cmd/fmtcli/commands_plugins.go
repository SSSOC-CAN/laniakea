...
// pluginStartRecord is the proxy command between fmtcli and the gRPC equivalent
func pluginStartRecord(ctx *cli.Context) error {
	...
	startRecRequest := &fmtrpc.PluginRequest{
		Name: pluginName,
	}
...
// pluginStopRecord is the proxy command between fmtcli and the gRPC equivalent
func pluginStopRecord(ctx *cli.Context) error {
	...
	defer cleanUp()
	pluginName := ctx.Args().First()
	stopRecRequest := &fmtrpc.PluginRequest{
		Name: pluginName,
	}
...
// pluginStart is the proxy command between fmtcli and the gRPC equivalent
func pluginStart(ctx *cli.Context) error {
	...
	defer cleanUp()
	pluginName := ctx.Args().First()
	startRequest := &fmtrpc.PluginRequest{
		Name: pluginName,
	}
...
// pluginStop is the proxy command between fmtcli and the gRPC equivalent
func pluginStop(ctx *cli.Context) error {
	...
	defer cleanUp()
	pluginName := ctx.Args().First()
	stopRequest := &fmtrpc.PluginRequest{
		Name: pluginName,
	}
...
// pluginCommand is the proxy command between fmtcli and the gRPC equivalent
func pluginCommand(ctx *cli.Context) error {
	...
	cmdRequest := &fmtrpc.ControllerPluginRequest{
		Name: pluginName,
		Frame: &proto.Frame{
			Source:    "fmtcli",
			Type:      frameType,
...
// addPlugin is the proxy command between fmtcli and the gRPC equivalent
func addPlugin(ctx *cli.Context) error {
	...
	addRequest := &fmtrpc.PluginConfig{
		Name:        pluginName,
		Type:        plugType,
		ExecName:    execName,
...
// listPlugins is the proxy command between fmtcli and the gRPC equivalent
...
// getPlugin is the proxy command between fmtcli and the gRPC equivalent
func getPlugin(ctx *cli.Context) error {
	...
	defer cleanUp()
	pluginName := ctx.Args().First()
	getRequest := &fmtrpc.PluginRequest{
		Name: pluginName,
	}
