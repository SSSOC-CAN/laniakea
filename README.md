...
To setup an NGINX reverse proxy for the gRPC API, create a file in `/etc/nginx/sites-enabled/fmtd-proxy.conf`:
```nginx
upstream fmtd_grpc {
...
server {
    ...
    ssl_certificate /path/to/fmtd_tls.cert; # Make sure this is the TLS certificate generated by the fmtd
    ssl_certificate_key /path/to/fmtd_tls.key; # Make sure this is the TLS certificate key generated by the fmtd
    location / {
        grpc_pass grpcs://fmtd_grpc;
    }
}
```
...
server {
    ...
    ssl_certificate_key /path/to/fmtd_tls.key; # Make sure this is the TLS certificate key generated by the fmtd
    location / {
        proxy_pass https://fmtd_rest;
        proxy_buffering off; # To make sure SubscribeDataStream responses aren't truncated
    }
}
```
